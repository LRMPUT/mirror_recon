...
load_path = '/home/rafal/Models/ral_3d_gen/furnitures/5ch'

images_save_path = '/home/rafal/Test/ral_3d_gen/furnitures/5ch'
...
for i, d in enumerate(tqdm(test_laoder)):
    ...
    results_data['rgb'].append(image_from_tensor(inputs['rgb'][0]))
    results_data['depth'].append(image_from_tensor(inputs['depth'][0]))
    results_data['depth_proj'].append(image_from_tensor(inputs['depth_proj'][0]))
    results_data['depth_back'].append(image_from_tensor(targets['depth_back'][0]))
    results_data['depth_pred'].append(image_from_tensor(predictions['depth_pred'][0]))
    ...
    # test_step += 1
    #
    # if log_step(test_step):
    #     predictions = dictify_output(pred.cpu())
    #     metrics = dictify_metrics(loss, l1_loss, ssim_loss)
    #
    #     test_logs.update(inputs, targets, predictions, metrics, test_step)
...
for k in l1_losses_per_class.keys():
    print(
        'Object: {}, Mean loss: {}, Count: {}'.format(k, np.mean(l1_losses_per_class[k]), len(l1_losses_per_class[k])))
